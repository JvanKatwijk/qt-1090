...
// Configure LO channel
	...
	plutoHandler::plutoHandler  (QSettings *s, int freq):
	                                  myFrame (nullptr),
	...
	rx_cfg. bw_hz			= 2400000;
	rx_cfg. fs_hz			= 2400000;
	rx_cfg. lo_hz			= this -> frequency;
...
//	   fprintf (stderr, "enabling filter failed\n");
	...
	fprintf (stderr, "Handler will be deleted\n");
	myFrame. hide ();
	plutoSettings	-> beginGroup ("plutoSettings");
...
void	plutoHandler::stopDevice () {
	if (!running. load())
	   return;
	running. store (false);
	while (isRunning())
	   usleep (500);
	fprintf (stderr, "Device is gestopt\n");
}
...
int	index	= 0;
	...
	while (running. load ()) {
	   ...
	   for (p_dat = (char *)iio_buffer_first (rxbuf, rx0_i);
	      ...
	      std::complex<float>sample = std::complex<float> (i_p, q_p);
...
int	plutoHandler::nrBits () {
	return 12;
}

void	plutoHandler::signalData	() {
	emit dataAvailable ();
}

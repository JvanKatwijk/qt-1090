...
lms_info_str_t limedevices [10];
	...
	limeHandler::limeHandler (QSettings *s, int freq):
	                             myFrame (nullptr),
...
//      From here we have a library available
	...
	res	= LMS_SetSampleRate (theDevice, 2400000.0, 0);
	if (res < 0) {
	   LMS_Close (theDevice);
	   throw (25);
	}
...
void	limeHandler::startDevice	() {
int	res;

	if (isRunning ())
	   return;
	LMS_SetLOFrequency (theDevice, LMS_CH_RX, 0, this -> frequency);
	stream. isTx            = false;
        stream. channel         = 0;

...
#include	<QSettings>
	...
	coordinates::coordinates	(QSettings *qt1090Settings) {
...
void	coordinates::handle_acceptButton () {
	if (!latitude || !longitude)
	   return;
	qt1090Settings	-> setValue ("latitude", latitude -> text ());
	qt1090Settings	-> setValue ("longitude", longitude -> text ());
	QDialog::done (0);
}

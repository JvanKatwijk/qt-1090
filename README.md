...
Dump1090-sdrplay is a Mode S decoder derived from Dump1090, that was
designed for RTLSDR devices. 
The current version supports SDRplay devices, as well as RTLSDR devices.
Device handling is completely decoupled from the main processing loop
and some more restructuring was done.
...
*	Interactive command-line-interfae mode where aircrafts
	currently detected are shown as a list refreshing as more
	data arrives.
...
* mkdir build
* cd build
* cmake .. -DXXX=ON (where XXX is either SDRPLAY or RTLSDR)
* make
* sudo make install
...
However, be sure that the program is run in the same directory as the file
gmap.html
...
In this example for`rtl_sdr` a gain of 50 is used, simply you should use the highest
